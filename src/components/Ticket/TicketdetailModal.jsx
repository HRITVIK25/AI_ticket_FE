...
import {
  Box, Typography, Button, Chip, Divider, Dialog, CircularProgress,
} from '@mui/material';
...
// ─────────────────────────────────────────────
const TicketDetailModal = ({ ticket: initialTicket, open, onClose, isGeneratingAi = false, onTicketUpdated }) => {
  const [ticket, setTicket] = useState(initialTicket);
  const [statusUpdating, setStatusUpdating] = useState(null); // 'CLOSED' | 'ASSIGNED' | null
      ...
      {ticket && (
        <Box sx={{ display: 'flex', flexDirection: { xs: 'column', md: 'row' }, height: '100%' }}>
          ...
          <Box sx={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
            ...
            {/* Action buttons — shown once AI has replied and ticket is still actionable */}
            {!needsPolling(ticket) && !isGeneratingAi && ticket.status !== 'CLOSED' && ticket.status !== 'ASSIGNED' && (
              <Box sx={{
                px: 3, py: 2.5,
