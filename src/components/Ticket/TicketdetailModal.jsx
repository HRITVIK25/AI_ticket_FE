...
import {
  Box, Typography, Button, Chip, Divider, Dialog, CircularProgress, TextField, IconButton,
} from '@mui/material';
...
// ─────────────────────────────────────────────
const TicketDetailModal = ({ ticket: initialTicket, open, onClose, isGeneratingAi = false, onTicketUpdated, isAdmin = false }) => {
  ...
  const sendMessage = useCallback(async () => {
    ...
    setTicket(prev => ({ ...prev, messages: [...(prev.messages || []), optimisticMsg] }));
    setChatMessage('');
    setIsSending(true);

    try {
      const res = await api.post(`api/v1/tickets/${ticket.id}/messages`, { message: text });
      if (res.data) {
        setTicket(res.data);
            ...
            <Box sx={{ mt: 'auto', pt: 1, display: 'flex', flexDirection: 'column', gap: 1.5 }}>
              {isAdmin && ticket.status !== 'CLOSED' && (
                <Button
                  ...
                  startIcon={statusUpdating === 'CLOSED' ? <CircularProgress size={16} sx={{ color: '#fff' }} /> : <CheckCircleOutlineIcon />}
                  sx={{
                    textTransform: 'none',
                    fontWeight: 600,
                    borderRadius: '10px',
                    ...
                    '&:hover': { background: 'linear-gradient(135deg, #059669, #047857)', transform: 'translateY(-1px)', boxShadow: '0 6px 20px -4px rgba(16, 185, 129, 0.5)' },
          ...
          <Box sx={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
            ...
            <Box sx={{ flex: 1, overflowY: 'auto', p: 3, display: 'flex', flexDirection: 'column', gap: 2 }}>
              ...
              {isAssigned && !isAdmin && (
                <Box sx={{
                  ...
                  <AccessTimeIcon sx={{ color: '#f59e0b', fontSize: '1.1rem', mt: '1px', flexShrink: 0 }} />
                  <Typography variant="caption" sx={{ color: '#fbbf24', lineHeight: 1.6 }}>
                    Your ticket has been assigned to a support agent.
                    They will reply as soon as possible — response times may vary based on agent availability.
                  </Typography>
                </Box>
              )}
                  ...
                  <Box key={idx} sx={{ display: 'flex', alignItems: 'flex-end', gap: 1.5, flexDirection: isCust ? 'row-reverse' : 'row' }}>
                    ...
                    <Box sx={{ maxWidth: '70%' }}>
                      <Typography variant="caption" sx={{ color: '#64748b', display: 'block', mb: 0.4, textAlign: isCust ? 'right' : 'left' }}>
                        {isAI ? 'AI Assistant' : isAgent ? 'Support Agent' : 'You'} · {time}
                      </Typography>
                      <Box sx={{
                        px: 2, py: 1.5,
                        ...
                        <Typography variant="body2" sx={{ color: isCust ? '#fff' : isAgent ? '#fcd34d' : '#cbd5e1', lineHeight: 1.6, whiteSpace: 'pre-wrap' }}>
                          {msg.message}
                        </Typography>
                      </Box>
                    </Box>
            ...
            {isAssigned && isAdmin && (
              <Box sx={{
                ...
                background: 'linear-gradient(180deg, rgba(15,23,42,0.7), rgba(10,18,30,0.9))',
              }}>
                <Typography variant="caption" sx={{ color: '#475569', display: 'block', mb: 1, pl: 0.5 }}>
                ...
                <Box sx={{ display: 'flex', gap: 1.5, alignItems: 'flex-end' }}>
                  ...
                  <IconButton
                    ...
                    sx={{
                      ...
                      background: chatMessage.trim() && !isSending
                        ? 'linear-gradient(135deg, #6366f1, #3b82f6)'
                        : 'rgba(30,41,59,0.5)',
                      color: chatMessage.trim() && !isSending ? '#fff' : '#475569',
                      borderRadius: '12px',
                      transition: 'all 0.2s',
                      '&:hover': {
                ...
                <Typography variant="caption" sx={{ color: '#334155', display: 'block', mt: 0.75, pl: 0.5 }}>
            ...
            {!needsPolling(ticket) && !isGeneratingAi && ticket.status !== 'CLOSED' && !isAssigned && (
              <Box sx={{
                px: 3, py: 2.5,
                ...
                <Button
                  fullWidth
                  variant="outlined"
                  startIcon={statusUpdating === 'CLOSED' ? <CircularProgress size={14} sx={{ color: '#94a3b8' }} /> : <CheckCircleOutlineIcon />}
                  disabled={!!statusUpdating}
                  onClick={() => updateStatus('CLOSED')}
                  sx={{
                    textTransform: 'none', fontWeight: 600, borderRadius: '12px',
                    color: '#94a3b8', borderColor: 'rgba(148,163,184,0.25)',
                    '&:hover': { borderColor: '#94a3b8', background: 'rgba(148,163,184,0.08)', color: '#f1f5f9' },
                ...
                <Button
                  fullWidth
                  variant="contained"
                  startIcon={statusUpdating === 'ASSIGNED' ? <CircularProgress size={14} sx={{ color: '#fff' }} /> : <EscalatorWarningIcon />}
                  disabled={!!statusUpdating}
                  ...
                  sx={{
                    ...
                    '&:hover': { background: 'linear-gradient(135deg, #d97706, #dc2626)', transform: 'translateY(-1px)', boxShadow: '0 8px 20px -4px rgba(239,68,68,0.5)' },
                    '&.Mui-disabled': { opacity: 0.5 },
                    transition: 'all 0.2s',
                  }}
                >
